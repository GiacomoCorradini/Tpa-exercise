...
string pair2str(const std::pair<int,int>& shape) {
    return std::to_string(shape.first) + string(",") + std::to_string(shape.second);
}
...
std::pair<int,int> str2pair(string s){
    int i = s.find(",");
    return {std::stoi(s.substr(0,i)), std::stoi(s.substr(i+1))};
}

/**
 * RETURN the nonzero cells as triplets in a string like
 * {{1,4,2},{3,5,7},{2,1,5},{3,5,1}}
*/
...
class Matrix {
    public:
        ...
        */
        ...
        Matrix( const int n, const int m, const Triplet<T> triplets[], const int triplets_size){
